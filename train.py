...
def train_cnn_rnn():
    ...
    with graph.as_default():
        ...
        with sess.as_default():
            logging.critical("params['l2_reg_lambda'] {}".format(params['l2_reg_lambda']))
            cnn_rnn = TextCNNRNN(
            ...
            logging.critical('Training starts here')

            logging.critical('x_train {}  '.format(x_train))
            logging.critical('y_train {}  '.format(y_train))
            logging.critical('batch_size {}  '.format(params['batch_size']))
            logging.critical('num_epochs {}  '.format(params['num_epochs']))
            logging.critical('evaluate_every {}'.format(params['evaluate_every']))
            logging.critical('list {}  '.format(list(zip(x_train, y_train))))
            logging.critical('list {}  '.format(len(list(zip(x_train, y_train)))))
            # Training starts here
            train_batches = data_helper.batch_iter(list(zip(x_train, y_train)), params['batch_size'],
                                                   params['num_epochs'])
            best_accuracy, best_at_step = 0, 0
            logging.critical('train_batches {}  '.format(train_batches))

            # Train the model with x_train and y_train
            for train_batch in train_batches:
                # logging.critical('train_batch {}  '.format(train_batch))
                x_train_batch, y_train_batch = zip(*train_batch)
                train_step(x_train_batch, y_train_batch)
                current_step = tf.train.global_step(sess, global_step)
                logging.critical('current_step {}  '.format(current_step))
                # Evaluate the model with x_dev and y_dev
                logging.critical(
                    'current_step % evaluate_every {}'.format(current_step % params['evaluate_every'] == 0))
                if current_step % params['evaluate_every'] == 0:
                    logging.critical('Evaluate the model with x_dev and y_dev')
                    ...
                    for dev_batch in dev_batches:
                        ...
                        logging.info('total_dev_correct: {}'.format(total_dev_correct))
